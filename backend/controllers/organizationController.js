...
export const createOrganizationController=async (req,res)=>{
  ...
  status:"provisioning"
    ...
    const workflowId = await triggerCreateOrganization({orgId:org._id.toString(), name, identifier, createdByEmail });
...
export const deleteOrganizationController=async(req,res)=>{
  ...
  try {
    const workflowId=await triggerDeleteOrganization({orgId,name,createdByEmail});
    res.status(200).json({
    ...
    res.status(500).json({ error: "Failed to start delete organization workflow" });
  }
}
