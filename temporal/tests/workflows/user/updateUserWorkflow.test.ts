...
describe("updateUserWorkflow", () => {
  ...
  it("should successfully update user name", async () => {
    ...
    expect(
      mockUpdateUserStatus
        .getCall(0)
        .calledBefore(mockUpdateUserInAuth0.getCall(0))
    ).to.be.true;
    expect(
      mockUpdateUserInAuth0
        .getCall(0)
  ...
  it("should successfully update user without organizationId", async () => {
    ...
    expect(
      mockUpdateUserStatus.getCall(0).calledWith(email, "updating", undefined)
    ).to.be.true;
    expect(
      mockUpdateUserStatus.getCall(1).calledWith(email, "active", undefined)
    ).to.be.true;
  });
  ...
  it("should handle blocked status variations", async () => {
    ...
    for (const testCase of testCases) {
      ...
      // Act
      const result = await updateUserWorkflow(email, {
      ...
      expect(
        mockUpdateUserInAuth0.calledWith(email, { blocked: testCase.blocked })
      ).to.be.true;
    }
  });
  ...
  it("should handle partial update failures with detailed error messages", async () => {
    ...
    const partialUpdateError = ApplicationFailure.create({
      message:
        "Failed to update password field in Auth0. Failed fields: password. Successful fields: name",
    ...
    try {
      ...
      expect(error.message).to.include(
        "Failed to update password field in Auth0"
      );
