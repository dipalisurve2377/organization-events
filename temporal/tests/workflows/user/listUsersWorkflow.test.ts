...
const listUsersWorkflow = async (
  ...
  try {
    console.log(
      `Starting list users workflow - page: ${page}, perPage: ${perPage}`
    );
    ...
    console.log(
      `List users workflow completed - found ${auth0Response.users.length} users`
    );
    return {
      success: true,
...
describe("listUsersWorkflow", () => {
  ...
  it("should successfully list users with custom pagination", async () => {
    ...
    expect(mockListUsersFromAuth0.calledWith(page, perPage, undefined)).to.be
      .true;
  });
  ...
  it("should handle Auth0 listing failure", async () => {
    ...
    expect(
      mockUpdateUserStatus
        .getCall(1)
        .calledWith("workflow", "failed", organizationId)
    ).to.be.true;
  });
  ...
  it("should handle edge case pagination values", async () => {
    ...
    for (const testCase of testCases) {
      ...
      const mockUsers = Array.from(
        { length: Math.min(testCase.perPage, testCase.total) },
        (_, i) => ({
          id: `auth0|${i + 1}`,
          email: `user${i + 1}@example.com`,
          name: `User ${i + 1}`,
  ...
  it("should handle Auth0 timeout and retry scenarios", async () => {
    ...
    try {
      ...
      expect(error.message).to.include(
        "Auth0 request timeout while listing users"
      );
