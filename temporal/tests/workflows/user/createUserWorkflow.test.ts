...
describe("createUserWorkflow", () => {
  ...
  it("should successfully create a user with all parameters", async () => {
    ...
    expect(
      mockUpdateUserStatus
        .getCall(0)
        .calledBefore(mockCreateUserInAuth0.getCall(0))
    ).to.be.true;
    expect(
      mockCreateUserInAuth0
        .getCall(0)
        .calledBefore(mockSaveAuth0IdToMongoDB.getCall(0))
    ).to.be.true;
    expect(
      mockSaveAuth0IdToMongoDB
        .getCall(0)
  ...
  it("should successfully create a user without organizationId", async () => {
    ...
    expect(
      mockUpdateUserStatus
        .getCall(0)
        .calledWith(email, "provisioning", undefined, name)
    ).to.be.true;
    expect(
      mockUpdateUserStatus
        .getCall(1)
        .calledWith(email, "created", undefined, name)
    ).to.be.true;
  });
