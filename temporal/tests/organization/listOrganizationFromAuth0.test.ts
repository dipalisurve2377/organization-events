...
describe('listOrganizationFromAuth0', () => {
  ...
  beforeEach(() => {
    ...
    // Mock the service
    const orgAuth0ServiceMock = {
      getAuth0Token: getAuth0TokenStub
    };
  ...
  it('should handle Auth0 client errors (4xx) as non-retryable', async () => {
    ...
    try {
      ...
      expect((error as Error).message).to.include('Auth0 responded with status 400');
    }
  });
  ...
  it('should handle Auth0 server errors (5xx) as retryable', async () => {
    ...
    try {
      ...
      expect((error as Error).message).to.include('Auth0 responded with status 500');
    }
  });
