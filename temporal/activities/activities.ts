...
const Organization=mongoose.models.Organization || mongoose.model('Organization',organizationSchema);
    ...
    export const createOrganizationInAuth0= async(name:string,identifier:string,createdByEmail:string):Promise<void>=>{

        try {
            
            const token= await getAuth0Token();

            console.log("Sending organization to Auth0",{name,identifier,createdByEmail});


            const orgRes= await axios.post(`https://dev-kfmfhnq5hivv164x.us.auth0.com/api/v2/organizations`,{
            ...
            await Organization.findOneAndUpdate({identifier},{auth0Id},{new:true})

            console.log(`Auth0 ID saved to db for identifier ${identifier} : ${auth0Id}`)

             return auth0Id;

        } catch (error:any) {
            console.error("Failed to create organization in Auth0:", error.response?.data || error.message);
 ...
 export const updateOrganizationStatus = async (
  ...
  console.log(`Organization status updated to "${status}" for ${orgId}`);
    ...
    export const updateOrganizationInAuth0=async(orgId?:string,name?:string,identifier?:string):Promise<void>=>{
       ...
       try {
          ...
          const response= await axios.patch(`https://dev-kfmfhnq5hivv164x.us.auth0.com/api/v2/organizations/${org.auth0Id}`,updatePayload,{
            headers:{
                Authorization: `Bearer ${token}`,
         ...
         console.log("Organization updated in MongoDB", dbUpdate);
          ...
          console.error("Failed to update Organization in Auth0", error.response?.data || error.message);
   ...
   export const getOrganizationNameById = async (orgId: string): Promise<string | null> => {
   await connectDB();
   const org = await Organization.findById(orgId);
...
export const deleteOrganizationInAuth0=async(orgId:string):Promise<void>=>{
    ...
    await axios.delete(`https://dev-kfmfhnq5hivv164x.us.auth0.com/api/v2/organizations/${org.auth0Id}`,{
        headers:{
            Authorization: `Bearer ${token}`,
        }
    })
