...
const updateUserStatus = async (
  ...
  try {
    console.log(`Updating user status - email: ${email}, status: ${status}, organizationId: ${organizationId || 'none'}, name: ${name || 'unchanged'}`);
    ...
    const validStatuses = ['active', 'inactive', 'pending', 'deleted', 'suspended', 'created', 'updated'];
    ...
    if (result.modifiedCount === 0) {
      ...
      console.log(`Successfully updated user status to ${status} for email: ${email}`);
    }
  } catch (error: any) {
...
describe('updateUserStatus', () => {
  ...
  it('should handle invalid status value', async () => {
    ...
    try {
      ...
      expect(error.message).to.include('Valid values are: active, inactive, pending, deleted, suspended, created, updated');
    }
  ...
  it('should handle no modifications (modifiedCount = 0)', async () => {
    ...
    const email = 'test@example.com';
    const status = 'active';
    const mockUser = { _id: 'user123', email, name: 'Test User', status: 'active' };
