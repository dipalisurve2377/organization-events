  ...
  try {
    ...
    if (deletedFrom.length > 0) {
      try {
        ...
        console.warn(`Failed to update status to deleted for ${email}:`, statusError);
      }
    }
...
describe('deleteUserWorkflow', () => {
  ...
  it('should successfully delete user from both Auth0 and MongoDB', async () => {
    ...
    expect(mockUpdateUserStatus.getCall(0).calledBefore(mockDeleteUserFromAuth0.getCall(0))).to.be.true;
    expect(mockDeleteUserFromAuth0.getCall(0).calledBefore(mockDeleteUserFromDB.getCall(0))).to.be.true;
    expect(mockDeleteUserFromDB.getCall(0).calledBefore(mockUpdateUserStatus.getCall(1))).to.be.true;
  });
  ...
  it('should successfully delete user without organizationId', async () => {
    ...
    expect(mockUpdateUserStatus.getCall(0).calledWith(email, "deleting", undefined)).to.be.true;
    expect(mockUpdateUserStatus.getCall(1).calledWith(email, "deleted", undefined)).to.be.true;
  });
  ...
  it('should handle MongoDB deletion failure (non-not-found error)', async () => {
    ...
    try {
      ...
      expect(error.message).to.include('MongoDB connection error during deletion');
    }
  ...
  it('should handle final status update failure gracefully', async () => {
    ...
    mockDeleteUserFromAuth0.resolves();
    mockDeleteUserFromDB.resolves();
    mockUpdateUserStatus.onCall(1).rejects(new Error('Status update to deleted failed'));
